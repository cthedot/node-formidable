...
function IncomingForm() {
  var possibles = [process.env.TMP, process.env.TEMP, '/tmp', process.cwd()],
      i = 0;
  ...
  // check tempdir possibilities if present  
  while (i < possibles.length) {
  	if (fs.statSync(possibles[i]).isDirectory()) {
  		this.uploadDir = possibles[i]
  		break 
  	};
  	i++;
  }
